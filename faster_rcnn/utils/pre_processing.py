...
def generate_gt_tensors(anchor_list,
    ...
    cls_shape = (feature_map_width, feature_map_height, 2 * num_anchors)
    mask_cls = np.zeros(shape=cls_shape)
    y_true_cls = np.zeros(shape=cls_shape)

    reg_shape = (feature_map_width, feature_map_height, 4 * num_anchors)
    mask_reg = np.zeros(shape=reg_shape)
    ...
    for anchor in anchor_list:
        ...
        prob_start_index = 2 * (area_index * len(anchors_area_list) + aspect_ratio_index)
        prob_index_mask = [prob_start_index, prob_start_index + 1]

        mask_cls[x_index][y_index][prob_index_mask] = 1
        y_true_cls[x_index][y_index][pred_box_start_index] = int(anchor.label)
        y_true_cls[x_index][y_index][pred_box_start_index + 1] = 1 - int(anchor.label)
        ...
        mask_reg[x_index][y_index][pred_box_index_mask] = 1
        y_true_reg[x_index][y_index][pred_box_index_mask] = anchor.reg_vector


    # Convert numpy arrays to tensors
    mask_cls = tf.convert_to_tensor(value=mask_cls,
                                    dtype=tf.bool)

    y_true_cls = tf.convert_to_tensor(value=y_true_cls,
                                      dtype=tf.float32)

    mask_reg = tf.convert_to_tensor(value=mask_reg,
                                    dtype=tf.bool)
    ...
    return mask_cls, y_true_cls, mask_reg, y_true_reg
...
def pre_process_img(image_width,
    ...
    for x_anchor_center_in_fm in range(feature_map_width):
        for y_anchor_center_in_fm in range(feature_map_height):
            ...
            x_anchor_center_in_image = (image_width / feature_map_width) * x_anchor_center_in_fm
            y_anchor_center_in_image = (image_height / feature_map_height) * y_anchor_center_in_fm

            for anchor_area in anchors_area_list:
                for anchor_aspect_ratio in anchors_aspect_ratio_list:


                    anchor = Anchor(area_in_pixels=anchor_area,
